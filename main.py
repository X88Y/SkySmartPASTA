...
import os
########
dir_ssmart_data = os.getenv('APPDATA')[0:-8] +'\\local'+'\\sky_logg_pass.txt'
with open(dir_ssmart_data, "r", encoding='utf_8') as ssmartdata:
    auth_token = ssmartdata.read().split('\n')
auth_token = 'Bearer '+str(requests.post('https://api-edu.skysmart.ru/api/v2/auth/auth/student', data='{"phoneOrEmail":"'+auth_token[0]+'","password":"'+auth_token[1]+'"}').json()["jwtToken"])
########
